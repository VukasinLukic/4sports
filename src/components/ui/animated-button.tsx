...
export default function AnimatedButton({ 
  ...
  return (
    <>
      ...
      <button
        className={`${animated ? 'rotation-animation animated-btn-primary' : ''} 
                   ${!animated ? (isPrimary 
                     ? 'bg-transparent border-2 border-green-500 text-green-500 hover:bg-green-500/10 hover:border-green-400 hover:shadow-lg hover:shadow-green-500/20' 
                     : 'bg-transparent border-2 border-white/30 text-white hover:bg-white/10 hover:border-white/50 hover:shadow-lg hover:shadow-white/10'
                   ) : ''}
      ...
      >
        <span className={`pointer-events-none flex h-12 flex-nowrap items-center gap-3 rounded-xl px-8 py-4 
                         ...
                         ${animated ? 'bg-black/80 text-primary' : ''}`}>
          <span>{children}</span>
        </span>
      </button>
    </>
